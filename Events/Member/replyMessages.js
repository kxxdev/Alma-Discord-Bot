...
const moderRoles = ['1065927076357623819', '920384487207084094', '1071954387343261736', '809405849687818261'];
...
const delMessage = async (message, action, text) => {
    ...
    if (!referenceMessage) return message.reply({ content: 'Ошибка. Я не могу получить исходное сообщение.' }).catch(err => console.log(err));
    // Удаляем сообщение.
    ...
    let time = 0;
    let timeName = 'часов';
    if (action === 'мутч') { time = 1; timeName = 'час' }
    else if (action === 'мутчч') { time = 6; timeName = 'часов' }
    else if (action === 'мутд') { time = 24; timeName = 'часа' }

    // Проверяем причину.
    if (!text) text = 'Не указано.';

    await referenceMessage.member.send({ content: `**Ваше сообщение удалено. Администратор: <@${message.author.id}>**\nПричина:\n\`\`\`${text}\`\`\`\nВам выдан мут на **${time}** ${timeName}.\nТекст сообщения:\n\`\`\`${referenceMessage.content}\`\`\`` }).catch(err => { });

    await message.reply({ content: 'Сообщение удалено, пользователю отправлена информация.' }).then((message) => {
        timeoutDel(message);
...
const mute = async (member, time) => {
    await member.disableCommunicationUntil(Date.now() + time * 1000 * 60 * 60).catch(err => console.log(err));
}
...
export  default {
    ...
    async execute(message) {
        ...
        if (delMessageActions.includes(action) && message.member.roles.cache.hasAny(...moderRoles)) return delMessage(message, action, text);
    }
}